...
public class CompanyService : Service<Company>, ICompanyService
{
    ...
    public override void Validate(Company entity)
    {
        ...
        if (!entity.IsVirtual && (string.IsNullOrWhiteSpace(entity.Address) || string.IsNullOrWhiteSpace(entity.AddressNumber)))
        ...
        if (entity.ServicesOffered != null & entity.ServicesOffered!.Any())
            throw new ArgumentException("At least one service is required");
        if (entity.OpeningHours != null & entity.OpeningHours!.Any())
    ...
    public override async Task SaveAsync(Company entity)
    {
        try
        {
            ...
            if (entity.ImageBase64 != null && entity.ImageBase64.IsValid)
            {
                string url = await _imageUploadService.UploadImage(entity.ImageBase64);
                entity.SetImageUrl(new Url(url));
            }
    ...
    private async Task GenerateImage(Company entity)
    {
        ...
        **Business Categories:** {String.Join(", ", entity.Categories != null ? entity.Categories.Select(c => Enum.GetName(typeof(CategoryEnum), c.CategoryId)) : "")}.
        ...
        string base64Image = await _openAIService.GetBase64NewImageAsync(prompt.Replace("\n", " ").Replace("  ", ""));

        entity.SetGeneratedImage(new Base64(base64Image), prompt);
    }
